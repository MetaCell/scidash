...
print open(os.path.join(os.path.dirname(__file__),
                        'config.json')).read().decode('utf-8')
config = json.loads(
    open(os.path.join(os.path.dirname(__file__), 'config.json')).read()
)
...
def writeToPomXML(repo):
    ...
    for node in all_modules:
        ...
        if write_repo == False:
            ...
            pom.writexml(
                open(pompath, "w"), indent="	", addindent="	", newl=''
            )
...
def writeToPlan(repo):
    #Open geppetto.plan
    basepath = os.path.dirname(__file__)
    planpath = os.path.abspath(
        os.path.join(basepath, "..", "..", "geppetto.plan")
    )
    ...
    if write_artifact == False:
        ...
        plan.writexml(
            open(planpath, "w"), indent="	", addindent="	", newl='\n'
        )
...
def main(argv):
    ...
    if not os.path.isdir(target_dir):
        raise IOError(
            "Geppetto sources folder does not exist, create it and set the sourcesdir field in config.json: \""
            + target_dir + "\""
        )
        ...
        for repo in config['repos']:
            if repo['auto_install'] == "yes":
                ...
                if repository in yes:
                    subprocess.call(
                        ['git', 'clone', repo['url']], cwd=target_dir
                    )
                    writeToPomXML(repo)
                    writeToPlan(repo)
